...
export class SidekickStore {
  ...
  claimNextObserverDelivery(claimedAt: string): StoredObserverDelivery | null {
  ...
  observerInboxStatus(): ObserverInboxStatus {
    const totals = z
      .object({
  ...
  getSettingsAudit(revision: number): {
    revision: number;
    changedFields: string[];
    changedAt: string;
  } | null {
