 ...
 * A lower later sample starts a new monotonic window instead of poisoning the entire checkpoint;
 * Sidekick waits for that fresh window to satisfy the same evidence threshold before forecasting.
...
export function projectGlobalRewardRunRate(input: {
  ...
  for (let index = 1; index < samples.length; index += 1) {
    ...
    if (
      previous &&
      sample &&
      BigInt(sample.globalAccruedRewardsSats) < BigInt(previous.globalAccruedRewardsSats)
    ) {
      monotonicStart = index;
    }
  }
  const monotonicSamples = samples.slice(monotonicStart);
  const recoveredFromDecrease = monotonicStart > 0;
  const first = monotonicSamples[0];
  const last = monotonicSamples.at(-1);
  ...
  if (monotonicSamples.length < minimumObservations || sampleBlocks < minimumSampleBlocks) {
    return {
      status: "unavailable",
      reason: recoveredFromDecrease ? "non-monotonic-accrual" : "insufficient-samples",
    };
  }

  const rates: Rate[] = [];
  const useCalculationAnchor = !firstCalculation && !recoveredFromDecrease;
  let previousHeight = useCalculationAnchor ? lastComputeHeight : first.observedBurnBlockHeight;
  let previousAccrued = useCalculationAnchor ? 0n : BigInt(first.globalAccruedRewardsSats);
  for (const sample of useCalculationAnchor ? monotonicSamples : monotonicSamples.slice(1)) {
  ...
  return {
    ...
    forecast: {
      ...
      sample: {
        observations: monotonicSamples.length,
      ...
      confidence: monotonicSamples.length >= 6 && sampleBlocks >= 24 ? "developing" : "low",
      assumptions: [
