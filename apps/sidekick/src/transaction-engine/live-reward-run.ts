import { ClarityType, cvToHex, noneCV, someCV, tupleCV, uintCV } from "@stacks/transactions";
import type {
  ManagerCapabilities,
...
async function registryStatus(
...
): Promise<"pending" | "accepted" | "rejected" | "unknown"> {
  const value = await context.node.callReadOnly(
    registry,
  ...
  const status = value.value.value.status;
  if (!status || status.type === ClarityType.OptionalNone) return "pending";
  if (status.type !== ClarityType.OptionalSome) return "unknown";
...
export function createLiveRewardRunFacts(options: LiveRewardRunFactsOptions) {
  return async (request: RewardRunPrepareRequest): Promise<RewardRunDraftFacts> => {
    ...
    while (accounts.length < maximumAccounts) {
      ...
      for (const candidate of page.candidates) {
        ...
        if (accounts.length >= maximumAccounts) break;
      }
    ...
    while (withdrawals.length < maximumAccounts) {
      ...
      for (const withdrawal of page.items) {
        const state = await registryStatus(
          context,
          sbtcRegistryContract,
          options.managerPrincipal,
...
export class LiveRewardRunDriver implements RewardRunDriver {
  ...
  async materialize(
    ...
    if (
      observation.setup.manager.managerPrincipal !== input.run.recipe.managerPrincipal ||
      observation.setup.manager.source.sha256 !== input.run.recipe.managerSourceFingerprint ||
      observation.setup.preflight.pox.pox5ContractId !== input.run.recipe.pox5Contract ||
      observation.setup.preflight.pox.sourceSha256 !== input.run.recipe.pox5SourceFingerprint ||
      observation.setup.preflight.pox.sbtcTokenContract !== input.run.recipe.sbtcTokenContract ||
      observation.setup.preflight.pox.sbtcRegistryContract !== input.run.recipe.sbtcRegistryContract
    ) {
      return { status: "halt", reason: "Reviewed contract identity changed after approval" };
    }
    if (
      input.child.operation !== "calculate-rewards" &&
      !reviewedRewardManagerAvailable(
        observation.setup.manager.capabilities,
        observation.setup.manager.source.sha256,
      )
    ) {
      return {
        status: "halt",
        reason: "The reviewed manager execution capability is unavailable",
      };
    }
