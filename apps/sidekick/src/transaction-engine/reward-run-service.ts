...
export interface RewardRunDraftFacts {
  ...
  /** Exact eligible work counts before the per-run transaction cap is applied. */
  eligibleAccountCount: number;
  eligibleWithdrawalCounts: { accepted: number; rejected: number };
...
export function buildRewardRunRecipe(input: {
  ...
  const eligibleTransactions =
    (selected.has("calculate-rewards") && input.facts.calculateRequired ? 1 : 0) +
    (selected.has("claim-rewards") && input.facts.collectRequired ? 1 : 0) +
    (selected.has("claim-staker-rewards") ? input.facts.eligibleAccountCount : 0) +
    (selected.has("settle-accepted-withdrawal")
      ? input.facts.eligibleWithdrawalCounts.accepted
      : 0) +
    (selected.has("reclaim-failed-withdrawal") ? input.facts.eligibleWithdrawalCounts.rejected : 0);
  if (eligibleTransactions < children.length) {
    throw new RewardRunError(
      "reward_run_invalid",
      "Eligible reward work is inconsistent with the sealed recipe",
    );
  }
  ...
  return {
    ...
    truncated: eligibleTransactions > children.length,
    remainingTransactions: eligibleTransactions - children.length,
...
export class RewardRunService {
  ...
  async #tick(runId: string): Promise<void> {
    ...
    const previousAttempt = this.#options.repository
      .attempts(runId, child.index)
      .find(({ precomputedTxid }) => precomputedTxid === signed.precomputedTxid);
    const attempt = previousAttempt
      ? this.#reuseRejectedAttempt(previousAttempt, runId, child.index, now.toISOString())
      : this.#options.repository.insertAttempt({
          runId,
          childIndex: child.index,
  ...
  #reuseRejectedAttempt(
    attempt: ReturnType<RewardRunRepository["attempts"]>[number],
    runId: string,
    childIndex: number,
    now: string,
    ...
    if (attempt.state !== "rejected") {
      throw new RewardRunError(
        "reward_run_conflict",
        "A signed transaction with this ID already exists and is not retryable",
      );
    }
    this.#options.repository.updateAttempt({
      runId,
    ...
    return { ...attempt, state: "signed", broadcastResult: null, updatedAt: now };
  }
