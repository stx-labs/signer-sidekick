...
(define-public (claim-staker-rewards
    ...
    (let (
        ...
        (try! (as-contract?
            ((with-ft 'ST1F7QA2MDF17S807EPA36TSS8AMEFY4KA9TVGWXT.sbtc-token
                "sbtc-token" earned
            ))
...
(define-public (reclaim-failed-withdrawal (request-id uint))
    (let (
            ...
            (request (unwrap!
                (contract-call?
                    'ST1F7QA2MDF17S807EPA36TSS8AMEFY4KA9TVGWXT.sbtc-registry
...
(define-public (sweep-fee-refunds (recipient principal))
    (let (
            (balance (unwrap-panic (contract-call? 'ST1F7QA2MDF17S807EPA36TSS8AMEFY4KA9TVGWXT.sbtc-token
                get-balance current-contract
            )))
