...
- Read every current pool-share input at one stable node anchor and expose a contract-exact
  `if-calculated-now` estimate with explicit assumptions and omission when evidence is incomplete.
- Add run-rate ranges, confidence/calibration, and realized-error history for forecasts.
...
- Anchored input discovery now reads the complete STX and active-bond share state at one index-block
  hash, runs the contract-proven simulator, and persists/exposes the result through Rewards and
