# Signer Sidekick v2.2.0

More reliable reward recovery, clearer payment history, and less background API and storage work.
This release includes database migrations and an event-observer configuration change; read
[Upgrade and recovery](#upgrade-and-recovery) before updating.
...
- Multiple instances can use separate Compose projects and published host ports; each project
  keeps its own data volume. Never share a database or gas wallet between running instances.
...
Set `SIDEKICK_IMAGE_TAG=2.2.0` and follow [Operations](../operator/operations.md#upgrade).
...
newer schemas: rollback needs a compatible database, and restoring must not discard a newer submission.
